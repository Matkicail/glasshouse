...
function cvPathSpec(p, colour) {
    ...
    return {
        ...
        layout: { ...LAYOUT_BASE, xaxis: { ...LAYOUT_BASE.xaxis, title: "log10 alpha", autorange: "reversed" }, yaxis: { ...LAYOUT_BASE.yaxis, title: "held-out mean deviance" } },
        table: { columns: ["alpha", "cv deviance", "se", "nonzero", ""], rows },
    };
}
function coefPathSpec(p) {
    const x = p.alphas.map((a) => Math.log10(a));
    const chosen = x[p.chosen] ?? 0;
    ...
    return {
        ...
        layout: { ...LAYOUT_BASE, xaxis: { ...LAYOUT_BASE.xaxis, title: "log10 alpha", autorange: "reversed" }, yaxis: { ...LAYOUT_BASE.yaxis, title: "coefficient (link scale)" }, legend: { orientation: "v", x: 1.02, y: 1 }, margin: { l: 56, r: 140, t: 36, b: 48 } },
        table: { columns: ["term", "coefficient at the chosen alpha"], rows },
    };
}
function gcvSpec(name, t, colour) {
    ...
    return {
        ...
        layout: { ...LAYOUT_BASE, xaxis: { ...LAYOUT_BASE.xaxis, title: "log10 lambda" }, yaxis: { ...LAYOUT_BASE.yaxis, title: "GCV" } },
...
function modelScreen(doc, root) {
    ...
    for (const m of labels) {
        ...
        const drawGcv = () => {
            ...
            if (t)
                renderChart(chart, gcvSpec(sel.value, t, colourOf(doc.models, m)));
        };
