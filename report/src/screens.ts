...
function thresholdScreen(doc: ReportDoc, root: HTMLElement): void {
  clear(root);
  ...
  const slider = el("input", { type: "range", min: "0", max: "100", value: "50", step: "1" }) as HTMLInputElement;
  ...
  const draw = () => {
    ...
    renderChart(chartBox, {
      ...
      layout: { ...LAYOUT_BASE, xaxis: { ...(LAYOUT_BASE.xaxis as object), title: "threshold", range: [0, 1] }, yaxis: { ...(LAYOUT_BASE.yaxis as object), range: [0, 1] } },
