...
| `uv sync` (again) after pulling a change to `pyproject.toml` | Installs new dependencies, including torch (CPU build, from the PyTorch index) for the research track | The research track is an extra for users (`pip install "glasshouse[research]"`); developers get it with the dev group |
